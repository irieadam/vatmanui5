...
module.exports = {
    ...
    doLogin: function (req, res, db) {
        ...
        db.user.authenticate(body).then(function (user) {
            ...
            var body = {"countryCode"    : userInstance.countryCode,
                        "vatNumber" : userInstance.vatNumber,
         ...
         db.user.findAll()
            .then(function (users) {
                ...
                for (var j in users) {
                    if (users[j].id !== 1 ) {
                        var userOut = {};
                        userOut.id=users[j].id;
                        userOut.email = users[j].email;
                        userOut.countryCode = users[j].countryCode ;
                        userOut.vatNumber = users[j].vatNumber;
                        userOut.name = users[j].name;
                        userOut.address = users[j].address;
                        userOut.createdAt = users[j].createdAt;
                        userOut.isAdmin = users[j].isAdmin;
                        userOut.validTo = users[j].validTo;
                        out.userList.push(userOut);
                    }
                }
    ...
    doExport: function (req, res, db) {
        ...
        var format =  req.query.format || 0; //1 = csv ,0 = excel 
        var fileName = '';
        var arrayOfDbResults = [];

        db.request.findAll({
            attributes: { exclude: ['id', 'sessionId', 'requestId', 'itemId', 'createdAt', 'status', 'requestDate', 'userId','error'] },
            ...
            .then(function (requests) {
                ...
                switch (format) {
                    ...
                    case '1' : //csv
                       ...
                       // res.setHeader('Content-Type', 'text/csv')//'text/plain',//application/vnd.openxmlformats-officedocument.spreadsheetml.sheet);
                        res.download(file);
                       res.status(200).set({
    ...
    callVatService: function (client, request) {
        return new Promise(function (resolve, reject) {
            var checkVatApprox = {
